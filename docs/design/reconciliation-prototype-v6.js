...
function switchTab(name) {
  document.getElementById('tab-income').classList.toggle('hidden', name !== 'income');
  document.getElementById('tab-prev').classList.toggle('hidden', name !== 'prev');
  document.getElementById('tab-btn-income').classList.toggle('active', name === 'income');
  document.getElementById('tab-btn-prev').classList.toggle('active', name === 'prev');
}

function updatePrevTabBadge() {
  const count = getPrevUnpaidRows().length;
  const badge = document.getElementById('prev-tab-badge');
  badge.textContent = count;
  badge.classList.toggle('show', count > 0);
}

function updatePayoutCountBadge() {
  const pending = payoutList.filter(p => p.status === '待匯款').length;
  const badge   = document.getElementById('payout-count-badge');
  badge.textContent = pending + ' 筆待匯款';
...
function getPrevUnpaidRows() {
  ...
  for (const [period, data] of Object.entries(PERIOD_DATA)) {
    ...
    for (const r of data) {
      if (r.status === '未匯款' || r.status === '部分匯款') rows.push(r);
    }
  }
...
function onPeriodChange() { clearSearch(); }
...
function clearSearch() {
  ['s-no', 's-master', 's-remitted', 's-date'].forEach(id => document.getElementById(id).value = '');
  ['s-status', 's-type'].forEach(id => document.getElementById(id).value = '');
...
function clearPrevSearch() {
  ['p-no', 'p-master', 'p-remitted', 'p-period', 'p-date'].forEach(id => document.getElementById(id).value = '');
  ['p-status', 'p-type'].forEach(id => document.getElementById(id).value = '');
  renderBlockPrev(getPrevUnpaidRows());
}
...
function addOrder(no, amount, isFull) {
  ...
  showToast(isFull ? '✓ 已全額加入匯款清單' : '✓ 已部分加入匯款清單', '#27ae60');
  refreshAll();
}
...
function validatePartialAmount() {
  ...
  else if (val > max)           error = `金額不可超過師傅剩餘收入 NT$ ${max.toLocaleString()}`;
...
function renderIncomeRow(r) {
  ...
  const statusClass = r.status === '已匯款'  ? 'status-remitted' :
                      r.status === '部分匯款' ? 'status-partial'  : 'status-unpaid';
...
function renderBlock1(rows) {
  ...
  if (rows.length === 0) {
    tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;color:#bbb;padding:30px">此週期無資料</td></tr>';
    return;
  }
...
function renderBlockPrev(rows) {
  ...
  if (rows.length === 0) {
    tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;color:#bbb;padding:30px">目前無前期未匯款項</td></tr>';
    return;
  }
  tbody.innerHTML = rows.map(r => renderIncomeRow(r)).join('');
}
...
function renderBlock3() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="8" class="payout-empty">此週期無匯款清單資料</td></tr>';
    return;
  }
  tbody.innerHTML = filtered.map(p => {
    const isPending   = p.status === '待匯款';
    const statusClass = isPending ? 'status-partial' : 'status-remitted';
