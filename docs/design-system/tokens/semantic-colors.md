...
| `Text/Inverse` | `Base/White` | `#FFFFFF` | 深色/照片底上需要跳脫預設深色文字時使用 |
...
| `Background/Inverse` | `Neutral/900` | `#2A2A2A` | 深色實色底，用於需要跳脫頁面淺色底的浮動元件 |
...
| Token | 對應 Primitive | 色值 | 使用情境 |
|-------|--------------|------|---------|
| `Status/Success` | `Green/600` | `#1AA354` | 成功狀態文字、圖示、badge、Tag 元件 Success tone 文字（原 Green/500 對比不足，2026-07-06 調整）|
| `Status/SuccessContainer` | `Green/100` | `#EDFFF4` | 成功狀態底色，Tag 元件 Success tone 底色（首次啟用場景）|
| `Status/Error` | `Red/400` | `#FF2851` | 錯誤文字、圖示、輸入框錯誤狀態、Tag 元件 Emphasis tone（Solid=true 底色 / Solid=false 文字）|
| `Status/ErrorContainer` | `Red/100` | `#FFE9ED` | 錯誤底色（Snackbar、錯誤提示區塊）、Tag 元件 Emphasis tone（Solid=false 底色，原 Red/150 調整，2026-07-06）|
...
| `Status/Error` 誤用 | 「師傅收入」金額 Tag 使用紅色做視覺強調，非錯誤語意 | ✅ Tag 元件備料時建立 Emphasis tone（Solid=true 對應此用例）|

---

## 觀察候選 token

目前無候選中的角色。（`Background/Notice` 已於 2026-07-06 因 Tag 元件 Notice tone 出現第二個使用場景，升級為正式 Background token）
